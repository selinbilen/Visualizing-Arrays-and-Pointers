...
s
	...
	printFormat("&f5:",formatPtrScalar(&f5,fmtFloat));
	printFormat("&vec1 pointer vector:",formatPtrVector(&vec1,4,fmtFloat));
	printFormat("matrix :",formatPtrMatrix(&matrix,3,4,fmtFloat));
	printFormat("pointer list:",formatPtrPlistScalar((void **)&plist2,5,fmtFloat));
	printFormat("&plistVector pointer to vector:",formatPtrPlistVector((void **)&plistVector,3,3,fmtFloat));
	...
	for(row=0;row<3 && compare;row++) {
		for(col=0;col<4 && compare ;col++) {
			if (prev!=NULL) if ((prev+1) !=(float *)&(plistPlist[row][col])) compare=0;
